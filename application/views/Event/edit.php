		...
		<div class="col-md-4">
			...
			<form name="createevent">
			  <div class="form-group">
			    ...
			    <input type="text" class="form-control" name="event_title" value="<?php echo $event['event_title'];?>" id="event_title">
			    <span id="event_title_error" class="alert alert-danger" style="display: none;"></span>
			  </div>
			  <div class="form-group">
			    <label for="start_date">Start Date:</label>
			    <input type="text" class="form-control" name="start_date" value="<?php echo $event['start_date'];?>" id="start_date">
			    <span id="start_date_error" class="alert alert-danger" style="display: none;"></span>
			  </div>
			  <div class="form-group">
			    <label for="end_date">End Date:</label>
			    <input type="text" class="form-control" name="end_date" value="<?php echo $event['end_date'];?>" id="end_date">
             ...
             ?>			  			
			  ...
			  <button type="button" class="btn btn-primary" onclick="checkValidation(event);">Update</button>
			</form>
		</div>
	</div>
</div>
...
<script type="text/javascript">
  ...
  function checkValidation(event){
  	...
  	$.ajax({
        url: '<?php echo base_url().'event/edit/' ?>'+event_id,
        type: 'post',
        dataType: "json",
        ...
        success: function (response) {
            if(response.success == true){
              window.location.replace("http://localhost/tatva/event");
